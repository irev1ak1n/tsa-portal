...
function Checklists({ app }) {
  const { myEvents, checklists, toggleChecklist, progressFor } = app;
    ...
    return (
      <div className="card">
        <Empty
          ...
          sub="Add an event and its deliverables checklist appears here automatically."
          action={
        ...
        return (
          <div className="section card" key={id}>
            ...
            <div style={{ marginTop: 10 }}>
              {items.map((it) => (
                <label className={`check ${it.done ? 'done' : ''}`} key={it.id}>
                  <input type="checkbox" checked={it.done} onChange={() => toggleChecklist(id, it.id)} />
                  <span>{it.label}</span>
                </label>
              ))}
            </div>
